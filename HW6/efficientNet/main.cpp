...
#include "models/efficientnet_mnist_int8_model_data.h"
...
int main() {
    print_line("=== EfficientNet MNIST INT8 (TFLite Micro) ===");

    static tflite::MicroErrorReporter micro_error_reporter;
    tflite::ErrorReporter* error_reporter = &micro_error_reporter;

    const tflite::Model* model = tflite::GetModel(efficientnet_mnist_int8_model_data);
    if (model->version() != TFLITE_SCHEMA_VERSION) {
        print_line("Model schema mismatch!");
        while (true) { ThisThread::sleep_for(1000ms); }
    }

    // Minimal resolver (FLASH korur)
    // Eğer "Failed to get registration from op code XYZ" alırsan,
    // XYZ'ye göre buraya 1 satır ekleyeceğiz.
    ...
    resolver.AddTanh();   // bazı graph’larda çıkabilir

    static constexpr int kTensorArenaSize = 350 * 1024;
    ...
    if (input->dims->size != 4) {
        print_line("Unexpected input dims!");
        while (true) { ThisThread::sleep_for(1000ms); }
    }

    if (input->dims->data[1] != 32 || input->dims->data[2] != 32 || input->dims->data[3] != 3) {
